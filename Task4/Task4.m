...
Y_dB_SNE = sne(dB, 2, PERPLEXITY + 70);
Y_dB_TSNE = tsne(dB, [], 2, [], PERPLEXITY);

GPLOT(Y_dB_PCA, dB_classes, [],'DB PCA');
GPLOT(Y_dB_SAMMON, dB_classes,[],'DB SAMMON');
GPLOT(Y_dB_SNE, dB_classes,[],'DB SNE');
GPLOT(Y_dB_TSNE, dB_classes,[],'DB TSNE');

PERPLEXITY = 15;
Y_dA_PCA = pca(dA, 2);
Y_dA_SAMMON = sammon(dA, 2);
Y_dA_SNE = sne(dA, 2, PERPLEXITY);
Y_dA_TSNE = tsne(dA, [], 2, [], 18);

GPLOT(Y_dA_PCA, dA_classes, dA_STR_LABELS,'DA PCA');
GPLOT(Y_dA_SAMMON, dA_classes,dA_STR_LABELS,'DA SAMMON');
GPLOT(Y_dA_SNE, dA_classes,dA_STR_LABELS,'DA SNE');
GPLOT(Y_dA_TSNE, dA_classes,dA_STR_LABELS,'DA TSNE');